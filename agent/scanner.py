...
class Scanner:
    ...
    def scan(self) -> ScanResponse:
        ...
        all_signals = []
        strategies_run = len(SWING_STRATEGIES)
    ...
    def _rank_and_aggregate(self, signals: List[Dict]) -> List[Recommendation]:
        ...
        for ticker, ticker_signal_list in ticker_signals.items():
            ...
            direction = "LONG" if len(long_strats) >= len(short_strats) else "SHORT"

            holding_period = ticker_signal_list[0]["holding_period"]
            trade_type = f"{direction} SWING"
        ...
        sp500_top10 = [ts for ts in ticker_scores if ts["is_sp500"]][:10]
        other_top10 = [ts for ts in ticker_scores if not ts["is_sp500"]][:10]
        
        final_scores = sp500_top10 + other_top10
