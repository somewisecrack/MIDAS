...
MIDAS is a state-of-the-art Swing Strategy Test Bench designed to backtest, analyze, and discover trading patterns across the stock market. Powered by an interactive web UI, robust Python backend, and Qwen-based local AI, MIDAS allows traders to batch backtest dozens of strategies across hundreds of tickers effortlessly.
...
- **AI-Powered Insights**: Integrated with Ollama/Qwen to automatically analyze backtest results, discover patterns, and answer questions via an interactive multi-modal chat window.
...
- Qwen Vision Model (run `ollama pull qwen2.5vl:32b` to use the primary local model)
...
4. **Analyze**: Use the right-side panel to view detailed trade logs, or click the **Chat** tab to ask the local Qwen AI to interpret your results and suggest optimizations.
